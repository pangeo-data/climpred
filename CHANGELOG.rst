...
- Fixed the weekly ``CI`` workflow silently skipping every test job. ``detect-ci-trigger`` only runs for ``push``/``pull_request``, so on ``schedule`` and ``workflow_dispatch`` the dependent matrix jobs were skipped and only the doctest and notebook jobs ran. `Aaron Spring`_
- The ``Upstream Test`` workflow now runs on Python 3.13. Upstream ``xclim`` requires ``>=3.11``, so the environment setup failed on Python 3.10 before any test could run. `Aaron Spring`_
- The ``Upstream Test`` workflow reports failures again: the removed ``::set-output`` syntax was replaced by ``$GITHUB_OUTPUT``, so the ``report`` job receives ``ARTIFACTS_AVAILABLE`` and opens/updates the upstream-CI failure issue. `Aaron Spring`_
- Updated the :py:meth:`.HindcastEnsemble.remove_bias` doctest values for ``how="modified_quantile"`` to match current `bias_correction` output. `Aaron Spring`_
- Removed a dangling always-true ``and "lead"`` clause in ``_rps`` that made the ``ty`` pre-commit hook fail. The condition is unchanged. `Aaron Spring`_
...
- `pytest-lazy-fixture` (abandoned) has been replaced by `pytest-lazy-fixtures` (maintained fork). (:pr:`870`) `Trevor James Smith`_
- GitHub Workflows have been adjusted to test against Python 3.13, as well as test `pip`-based environments. (:pr:`870`) `Trevor James Smith`_
