...
def _get_resample_func(ds):
    ...
    print("resample_func", resample_func.__name__)
...
def resample_skill_empty_dim(self, iterations, resample_dim, verify_kwargs):
    ...
    if resample_dim in dim:
        ...
        remaining_dim = []  # dim
        post_dim = dim
    print("remaining_dim", remaining_dim, "post_dim", post_dim, "dim", dim)
...
def resample_skill_resample_before(self, iterations, resample_dim, verify_kwargs):
    ...
    if "uninitialized" in verify_kwargs["reference"]:
        if not self.get_uninitialized():
            ...
            # faster generate once and then resample, see https://github.com/pangeo-data/climpred/blob/bec54bc03d432e64372253db5d61fa761c46d03c/climpred/bootstrap.py#L715
            ...
            if dask.is_dask_collection(copy_self._datasets["uninitialized"]):
                chunking_dims = [
                    d
                    for d in copy_self.get_initialized().dims
                    if d not in CLIMPRED_DIMS
                ]
