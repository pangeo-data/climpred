...
def _mean_bias_reduction_quick(hind, bias, dim):
    ...
    """
    bias_reduced_hind = hind.groupby(f'{dim}.dayofyear') - bias.groupby(f'{dim}.dayofyear').mean()
...
def _mean_bias_reduction_cross_validate(hind, bias, dim):
    ...
    for init in hind.init.data:
        ...
        bias_reduced_hind.append(
            hind.sel(init=init).groupby('init.dayofyear') - bias.sel(init=hind_drop_init_where_bias).groupby(f'init.dayofyear').mean()
        )
