...
def test_bootstrap_perfect_model_ds_not_nan(PM_ds_ds, PM_ds_control, metric, comparison):
    actual = bootstrap_perfect_model(
        PM_ds_ds, PM_ds_control, metric=metric, comparison=comparison, sig=50, bootstrap=2).isnull().any()
    assert actual == False
...
def test_bootstrap_perfect_model_da_not_nan(PM_da_ds, PM_da_control, metric, comparison):
    actual = bootstrap_perfect_model(
        PM_da_ds, PM_da_control, metric=metric, comparison=comparison, sig=50, bootstrap=2).isnull().any()
    assert actual == False
