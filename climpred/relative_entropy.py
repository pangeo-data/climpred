...
def rel_entropy(sigma_b, sigma_x, mu_x, mu_b, neofs):
    fac = 0.5
...
def _gen_control_ensemble(ds, control):
    """Generate a control ensemble as the control PDF Pb_c."""
    ds_list = []
    for _ in range(10):
...
def plot_rel_ent(rel_ent, rel_ent_threshold=None, **kwargs):
...
def bootstrap_relative_entropy(ds, control, control_ensemble, sig=99,
    ...
    for _ in range(min(1, int(bootstrap / ds['time'].size))):
        ...
        ds_pseudo_rel_ent = compute_relative_entropy(
            ds_control, control, control_ensemble, neofs=neofs, pcscaling=0,
            eofscaling=0, curv=curv, ntime=ds.time.size)
...
def compute_relative_entropy(ds, control, control_ensemble, neofs=5,
                             curv=True, ntime=10):
    """Compute relative entropy from a field."""
    ...
    for ens in ensembles:
        for t in ds.time.values[:ntime]:
            ...
            r, d, s = rel_entropy(sigma_b, sigma_x, mu_x, mu_b, neofs)

            rel_ent.T.loc['R', ens][t] = r
            rel_ent.T.loc['D', ens][t] = d
            rel_ent.T.loc['S', ens][t] = s
    return rel_ent.dropna()
